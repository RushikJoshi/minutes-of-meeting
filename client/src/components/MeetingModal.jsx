...
export default function MeetingModal({
    ...
    setForm(
      meeting
        ? {
          ...
          startTime: meeting.startTime ? new Date(meeting.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false }) : "",
        ...
        : { ...DEFAULT_FORM, date: initialDate || DEFAULT_FORM.date }
    );
    setParticipants(Array.isArray(meeting?.participants) ? meeting.participants : []);
    setError("");
  }, [open, meeting, initialDate]);

  const title = useMemo(() => (meeting ? "Edit meeting" : "Create meeting"), [meeting]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      // Combine date and startTime strings into a single Date object
      ...
      <div className="w-full max-w-3xl overflow-hidden rounded-[2rem] border border-white/70 bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-5">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Plan the meeting, invite participants, and schedule reminders.</p>
          </div>
          <button type="button" className="btn-secondary !px-4 !py-2" onClick={onClose} disabled={isSubmitting}>
            Close
          </button>
        </div>
